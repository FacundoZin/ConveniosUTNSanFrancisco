    ...
    public class SearchByNumeroConvenioCmd : IFilterCommands
    {
        ...
        public async Task<Result<object>> ExecuteAsync(_UnitOfWork _UnitOfWork)
        {
            ...
            else
            {
                ...
                var task1 = context1.ConveniosEspecificos.Where(c => c.numeroconvenio == _dto.NumeroConvenio)
                    .ToListAsync();

                var task2 = context2.ConveniosMarcos.Where(c => c.numeroconvenio == _dto.NumeroConvenio)
                    .ToListAsync();
