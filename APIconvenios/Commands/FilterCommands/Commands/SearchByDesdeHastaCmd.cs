...
namespace APIconvenios.Commands.FilterCommands.Commands
{
    public class SearchByDesdeHastaCmd : IFilterCommands
    {
        ...
        public SearchByDesdeHastaCmd(ConvenioQueryObject query)
        {
            _query = query;
            _Dto = query.ByDesdeHastaDto;
        }
        ...
        public async Task<Result<object>> ExecuteAsync(_UnitOfWork _UnitOfWork)
        {
            if(_Dto.convenioType == "marco")
            {
                var query = _UnitOfWork._ConvenioMarcoRepository.GetQueryByFiltering()
                    .Where(c => c.FechaFirmaConvenio.HasValue
                        && c.FechaFirmaConvenio.Value >= _Dto.desde
                        && c.FechaFirmaConvenio.Value <= _Dto.hasta);
            ...
            else
            {
                ...
                var q1 = context1.ConveniosEspecificos.Where(c => c.FechaFirmaConvenio.HasValue
                    && c.FechaFirmaConvenio.Value >= _Dto.desde
                    && c.FechaFirmaConvenio.Value <= _Dto.hasta);
                var q2 = context2.ConveniosMarcos.Where(c => c.FechaFirmaConvenio.HasValue
                    && c.FechaFirmaConvenio.Value >= _Dto.desde
                    && c.FechaFirmaConvenio.Value <= _Dto.hasta);
