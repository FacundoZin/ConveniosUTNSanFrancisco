...
namespace APIconvenios.Commands.FilterCommands.Commands
{
    public class SearchByFechaFirmaCmd : IFilterCommands
    {
        ...
        public SearchByFechaFirmaCmd(ConvenioQueryObject query)
        {
            _query = query;
            _Dto = query.ByFechaFirma;
        }
        ...
        public async Task<Result<object>> ExecuteAsync(_UnitOfWork _UnitOfWork)
        {
            if (_Dto.convenioType == "marco")
            {
                var query = _UnitOfWork._ConvenioMarcoRepository.GetQueryByFiltering()
                    .Where(c => c.FechaFirmaConvenio == _Dto.FechaInicio);
            ...
            else
            {
                ...
                var q1 = context1.ConveniosEspecificos.Where(c => c.FechaFirmaConvenio == _Dto.FechaInicio);
                var q2 = context2.ConveniosMarcos.Where(c => c.FechaFirmaConvenio == _Dto.FechaInicio);
