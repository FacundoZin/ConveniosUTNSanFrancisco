...
namespace APIconvenios.Commands.FilterCommands.Commands
{
    public class SearchByNumeroResolucionCmd : IFilterCommands
    {
        ...
        public SearchByNumeroResolucionCmd(ConvenioQueryObject query)
        {
            _query = query;
            _dto = query.ByNumeroResolucion;
        }
        public async Task<Result<object>> ExecuteAsync(_UnitOfWork _UnitOfWork)
        {
            if (_dto.convenioType == "marco")
            {
                var query = _UnitOfWork._ConvenioMarcoRepository.GetQueryByFiltering()
                    .Where(c => c.NumeroResolucion == _dto.NumeroResolucion);
            ...
            else
            {
                ...
                var q1 = context1.ConveniosEspecificos.Where(c => c.NumeroResolucion == _dto.NumeroResolucion);
                var q2 = context2.ConveniosMarcos.Where(c => c.NumeroResolucion == _dto.NumeroResolucion);
