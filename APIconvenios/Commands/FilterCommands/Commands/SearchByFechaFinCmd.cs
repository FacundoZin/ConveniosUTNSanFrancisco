...
namespace APIconvenios.Commands.FilterCommands.Commands
{
    public class SearchByFechaFinCmd : IFilterCommands
    {
        ...
        public SearchByFechaFinCmd(ConvenioQueryObject query)
        {
            _query = query;
            _Dto = query.ByFechaFin;
        }
        ...
        public async Task<Result<object>> ExecuteAsync(_UnitOfWork _UnitOfWork)
        {
            if (_Dto.convenioType == "marco")
            {
                var query = _UnitOfWork._ConvenioMarcoRepository.GetQueryByFiltering()
                    .Where(c => c.FechaFin == _Dto.FechaFin);
            ...
            else if (_Dto.convenioType == "especifico")
            {
                var query = _UnitOfWork._ConvenioEspecificoRepository.GetQueryByFiltering()
                    .Where(c => c.FechaFinConvenio == _Dto.FechaFin);
            ...
            else
            {
                ...
                var q1 = context1.ConveniosEspecificos.Where(c => c.FechaFinConvenio == _Dto.FechaFin);
                var q2 = context2.ConveniosMarcos.Where(c => c.FechaFin == _Dto.FechaFin);
