    ...
    <div class="offcanvas-body">
      <div v-if="typeOfConvenio === 'marco'">
        ...
        <div v-for="value in KeyFilters"></div>
          ...
          <li class="list-group-item" @click="selectFilter(KeyFilters.ByProximosAvencer)">Proximos a vencer</li>
          <li class="list-group-item" @click="selectFilter(KeyFilters.ByIsRefrendado)">Refrendados</li>
        </ul>
      </div>
      <div v-else-if="typeOfConvenio === 'especifico'">
        <ul class="list-group">
          ...
          <li class="list-group-item" @click="selectFilter(KeyFilters.ByProximosAvencer)">Proximos a vencer</li>
          <li class="list-group-item" @click="selectFilter(KeyFilters.ByIsRefrendado)">Refrendados</li>
          <li class="list-group-item" @click="selectFilter(KeyFilters.ByIsActa)">Actas</li>
        </ul>
      </div>
      <div v-else>
...
// Define los DOS eventos que puede emitir: cerrar el panel y seleccionar un filtro
const emit = defineEmits(['close-panel', 'filter-selected']);
...
const selectFilter = (filterKey: string) => {
    // 1. Notifica al Dashboard qué filtro se seleccionó
    emit('filter-selected', filterKey);
    // 2. Cierra el panel de menú
    emit('close-panel'); 
};
</script>
...
.offcanvas-backdrop {
  ...
  top: 0;
  left: 0;
  z-index: 1040; /* Z-index estándar de Bootstrap para overlays */
  width: 100vw;
  height: 100vh;
...
.offcanvas {
  max-width: 400px; /* Limita el ancho del panel */
  background-color: #f8f9fa; /* Color de fondo claro para que se distinga */
}
.list-group-item {
    cursor: pointer; /* Hace que se vea clickable */
}
</style>